...
function AdminContent() {
       ...
       const handleSaveProduct = async () => {
              ...
              const payload = { name: formData.name, price: Number(formData.price), cost: Number(formData.cost) || 0, stock: Number(formData.stock) || 0, category_id: catId, image_url: imageUrl, is_active: true };
       ...
       return (
              <div className="flex flex-col h-dvh grid-background overflow-hidden">
                     <div className="flex-none p-4 pt-safe flex items-center justify-between z-10">
                            ...
                            <Link href="/" className="glass-card px-3 py-2 rounded-full text-gray-600 font-bold flex items-center gap-1 hover:bg-white transition"><Home size={18} /> เมนู</Link>
                     </div>
                            ...
                            <div className="max-w-2xl mx-auto pb-20">
                                                 ...
                                                 {debtors.length === 0 ? <div className="text-center text-gray-400 py-10">ไม่มีคนติดหนี้</div> : debtors.map(order => (
                                                        <div key={order.id} className="glass-card bg-white p-4 rounded-2xl relative overflow-hidden">
                                                               ...
                                                               <div className="flex justify-between items-start mb-3">
                                                                      <div><h3 className="font-bold text-lg text-gray-800 flex items-center gap-2"><BookUser size={18} className="text-orange-500" /> {order.customer_name}</h3><p className="text-sm text-gray-500">{order.customer_contact || "ไม่มีเบอร์โทร"}</p><p className="text-xs text-gray-400 mt-1">วันที่: {new Date(order.created_at).toLocaleDateString('th-TH')}</p></div>
                                                                      <div className="text-right"><span className="block text-2xl font-extrabold text-orange-600">{order.total_amount}.-</span></div>
                                                               </div>
                                                               <div className="grid grid-cols-2 gap-3"><button onClick={() => handleRepay(order.id, 'CASH')} className="py-2 bg-green-600 text-white rounded-xl font-bold text-sm shadow-sm hover:bg-green-700 active:scale-95 transition">รับเงินสด 💵</button><button onClick={() => handleRepay(order.id, 'QR')} className="py-2 bg-blue-600 text-white rounded-xl font-bold text-sm shadow-sm hover:bg-blue-700 active:scale-95 transition">รับโอน/QR 📱</button></div>
                                                        </div>
                                                 ))}
                                          </div>
                                   )}
                                   {activeTab === 'quick' && (
                                          <div className="glass-card bg-white p-6 rounded-3xl">
                                                 ...
                                                 <div className="flex gap-2 mb-6"><input type="number" placeholder="ราคา" value={newQuickPrice} onChange={e => setNewQuickPrice(e.target.value)} className="flex-1 p-3 border rounded-xl outline-none focus:ring-2 focus:ring-blue-500" /><button onClick={handleAddQuickButton} className="bg-green-600 text-white px-4 rounded-xl font-bold active:scale-95 transition">เพิ่ม</button></div>
                     ...
                     {isEditing && (
                            <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
                                   <div className="glass-card bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-in zoom-in-95 duration-200">
                                          <div className="flex justify-between mb-4"><h3 className="font-bold text-lg">{editingId ? 'แก้ไขสินค้า' : 'เพิ่มสินค้า'}</h3><button onClick={() => setIsEditing(false)} className="bg-gray-100 p-1 rounded-full"><X size={20} /></button></div>
                                          <div className="space-y-3">
                                                 <div onClick={() => fileInputRef.current?.click()} className="h-32 bg-gray-50 border-2 border-dashed rounded-xl flex items-center justify-center cursor-pointer relative overflow-hidden hover:bg-gray-100 transition">
                                                        {imageFile ? (<img src={URL.createObjectURL(imageFile)} className="w-full h-full object-cover" />) : (formData.image_url ? (<img src={formData.image_url} className="w-full h-full object-cover" />) : (<span className="text-gray-400 text-sm flex flex-col items-center"><ImageIcon className="mb-1" /> เลือกรูปภาพ</span>))}
                                                 ...
                                                 <select className="w-full p-2 border rounded-lg outline-none bg-white focus:border-blue-500" value={formData.category_id} onChange={(e) => setFormData({ ...formData, category_id: e.target.value })}><option value="">-- เลือกหมวดหมู่ --</option>{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
                                                 ...
                                                 <button onClick={handleSaveProduct} className="w-full py-3 bg-blue-600 text-white rounded-xl font-bold mt-2 shadow-lg hover:bg-blue-700 active:scale-95 transition">{editingId ? 'บันทึกการแก้ไข' : 'เพิ่มสินค้า'}</button>
                                          </div>
                                   </div>
                            </div>
                     )}
