...
export default function AdminPage() {
       ...
       if (!isAuthenticated) return (
              <div className="h-[100dvh] flex flex-col items-center justify-center bg-gray-900 text-white">
       ...
       return (
              // ✅ เปลี่ยน Layout เป็น Flex Column เต็มจอ (h-[100dvh]) เหมือนหน้า POS
              <div className="flex flex-col h-[100dvh] bg-gray-100 overflow-hidden">

                     {/* 🟢 ส่วนหัว (Header) - Fix อยู่กับที่ */}
                     <div className="flex-none bg-white p-4 shadow-sm z-10 pt-safe">
                            <div className="flex justify-between items-center max-w-2xl mx-auto mb-4">
                                   <h1 className="text-xl font-bold text-gray-800">จัดการระบบ</h1>
                                   <Link href="/" className="text-sm bg-gray-100 px-3 py-1 rounded-lg hover:bg-gray-200">ออก</Link>
                            </div>

                            {/* เมนู Tabs */}
                            <div className="flex p-1 bg-gray-100 rounded-xl max-w-2xl mx-auto overflow-x-auto no-scrollbar">
                                   {['stock', 'debt', 'quick', 'settings'].map(tab => (
                                          <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 py-2 px-3 rounded-lg font-bold text-xs sm:text-sm flex justify-center items-center gap-1 whitespace-nowrap transition-all ${activeTab === tab ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}>
                     ...
                     {/* 🟢 ส่วนเนื้อหา (Content) - เลื่อนได้ (Scrollable) */}
                     <div className="flex-1 overflow-y-auto p-4 pb-safe">
                            <div className="max-w-2xl mx-auto pb-20">
                                   ...
                                   {activeTab === 'stock' && (
                                          <>
                                                 <button onClick={() => setIsEditing(true)} className="w-full py-3 bg-blue-600 text-white rounded-xl font-bold shadow-sm mb-4 flex justify-center items-center gap-2 hover:bg-blue-700 active:scale-95 transition">
                                                        <Plus /> เพิ่มสินค้า
                                                 </button>
                                                 <div className="grid gap-3">
                                                        {products.map(p => (
                                                               <div key={p.id} className="bg-white p-3 rounded-xl shadow-sm flex items-center gap-3 border border-gray-100">
                                                                      <div className="w-14 h-14 bg-gray-100 rounded-lg overflow-hidden relative border flex-shrink-0">
                                                                      ...
                                                                      <div className="text-right">
                                                                             ...
                                                                             <button onClick={() => handleArchive(p.id)} className="text-xs text-gray-300 hover:text-red-500 mt-1 flex items-center gap-1 justify-end"><Archive size={12} /> ปิด</button>
                                                                      </div>
                                                               </div>
                                                        ))}
                                                 </div>
                                                 ...
                                                 {debtors.length === 0 ? <div className="text-center text-gray-400 py-10">ไม่มีคนติดหนี้ เย้! 🎉</div> : debtors.map(order => (
                                                        <div key={order.id} className="bg-white p-4 rounded-xl shadow-sm border border-orange-100 relative overflow-hidden">
                                                               ...
                                                               <div className="grid grid-cols-2 gap-3">
                                                                      <button onClick={() => handleRepay(order.id, 'CASH')} className="py-2 bg-green-600 text-white rounded-lg font-bold text-sm shadow-sm hover:bg-green-700 active:scale-95 transition">รับเงินสด 💵</button>
                                                                      <button onClick={() => handleRepay(order.id, 'QR')} className="py-2 bg-blue-600 text-white rounded-lg font-bold text-sm shadow-sm hover:bg-blue-700 active:scale-95 transition">รับโอน/QR 📱</button>
                                                               </div>
                                                        </div>
                                                 ))}
                                          </div>
                                   ...
                                   {activeTab === 'settings' && (
                                          <div className="bg-white p-6 rounded-2xl shadow-sm">
                                                 ...
                                                 <label className="block text-sm text-gray-500 mb-2">PromptPay / รหัสบัตร ปชช.</label>
